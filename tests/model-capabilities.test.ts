  ...
  test('uses explicit relay model profiles for CatsCo relay vision capabilities', () => {
    assert.deepStrictEqual(
      RELAY_MODEL_PROFILES.map(profile => ({
        ...
        inputLimit: profile.maxInputTokens,
        outputLimit: profile.maxOutputTokens,
        input: profile.inputModalities,
        output: profile.outputModalities,
      })),
      [
        { model: 'MiniMax-M2.7', provider: 'anthropic', vision: false, context: 204_800, inputLimit: undefined, outputLimit: 131_072, input: ['text'], output: ['text'] },
        { model: 'MiniMax-M3', provider: 'anthropic', vision: true, context: 1_000_000, inputLimit: undefined, outputLimit: 128_000, input: ['text', 'image', 'video'], output: ['text'] },
        { model: 'deepseek-v4-flash', provider: 'anthropic', vision: false, context: 1_000_000, inputLimit: undefined, outputLimit: 384_000, input: ['text'], output: ['text'] },
        { model: 'gpt-5.6-terra', provider: 'openai', vision: true, context: 1_050_000, inputLimit: 922_000, outputLimit: 128_000, input: ['text', 'image', 'pdf'], output: ['text'] },
        { model: 'gpt-5.6-sol', provider: 'openai', vision: true, context: 1_050_000, inputLimit: 922_000, outputLimit: 128_000, input: ['text', 'image', 'pdf'], output: ['text'] },
        { model: 'gpt-5.6-luna', provider: 'openai', vision: true, context: 1_050_000, inputLimit: 922_000, outputLimit: 128_000, input: ['text', 'image', 'pdf'], output: ['text'] },
      ],
    );
